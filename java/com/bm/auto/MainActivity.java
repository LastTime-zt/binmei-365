...
public class MainActivity extends Activity {
    ...
    private void buildHttpBlock(final LinearLayout block, float d) {
        ...
        // 一键日常任务(账号登录卡内): 签到 + 随机练习10题 + 阅读15篇(浏览任务) + 模拟考试
        Button bDaily = mkBtn("一键日常+自动答题(签到+练习+阅读+模拟)", true);
        ...
        bUpdateCheck.setOnClickListener(new View.OnClickListener() {
            @Override public void onClick(View v) {
                if (mDlState == 1) {
                    ...
                    mDlState = 0;
                    sp.edit().putInt("dl_state", 0).apply();
                    resetDlUi();
    ...
    private void installPendingApk() {
        ...
        if (!apk.exists()) {
            apk = new File(getApplicationContext().getExternalFilesDir(null), "AutoAnswer_update.apk");
        }
        if (apk.exists()) {
    ...
    private void onCheckUpdateReady() {
        ...
        if (savedState == 2) {
            ...
            bUpdateCheck.setEnabled(true);
            bUpdateCheck.setText("安装更新");
            Toast.makeText(this, "检测到上次下载已完成，请点击按钮安装", Toast.LENGTH_LONG).show();
            return;
        }
        checkUpdate();
    }
    ...
    private void loadBankInfo() {
        ...
        if (appMode) {
            ...
            sb.append("\n使用步骤(方式二·协议答题):\n")
              ...
              .append("6. 「一键日常+自动答题」自动完成签到+练习+阅读+模拟考试\n\n")
